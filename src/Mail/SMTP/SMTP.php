 ...
 * Time: 23:40
 */
namespace Ceive\Messenger\Mail\SMTP {
	
	use Ceive\Messenger\CombinationInterface;
	use Ceive\Messenger\ContactInterface;
	use Ceive\Messenger\Mail\Contact;
	use Ceive\Messenger\Mail\ContactInterface as MailContactInterface;
	use Ceive\Messenger\Mail\MessageInterface;
	use Ceive\Messenger\Messenger;
	use Ceive\Net\Integrator\Stream\ExtraCombination;
	use Ceive\Net\Integrator\Stream\SmtpApi;
	...
	class SMTP extends Messenger{
		...
		protected function registerDestination(ContactInterface $destination){
			if($destination instanceof MailContactInterface){
				switch($destination->getType()){
					case MailContactInterface::TYPE_MAIN:
						if(count($this->to) < 1){
							$this->to[] = $destination;
						}else{
							$this->cc[] = $destination;
						...
						break;
					case MailContactInterface::TYPE_CC:
						$this->cc[] = $destination;
						break;
					case MailContactInterface::TYPE_BCC:
		...
		protected function sendCollection(){
			 ...
			 * @var Contact $sender
			 * @var Contact $author
			 * @var \Ceive\User\AccessAuth\Pair $auth
			...
			foreach($this->to as $d){
				$to[] = ($d instanceof MailContactInterface && $d->getName()?$this->prepareString($d->getName()):'') . " <{$d->getAddress()}>";
				$destinations[] = $d;
			}
			...
			foreach($this->cc as $d){
				$cc[] = ($d instanceof MailContactInterface && $d->getName()?$this->prepareString($d->getName()):'') . " <{$d->getAddress()}>";
				$destinations[] = $d;
			}
			...
			foreach($this->bcc as $d){
				$bcc[] = ($d instanceof MailContactInterface && $d->getName()?$this->prepareString($d->getName()):'') . " <{$d->getAddress()}>";
				$destinations[] = $d;
			}
